#' Show
#'
#' Show function for a \code{subRegressions} object
#'
#' @param x A \code{subRegressions} object
#'
#' @return NULL
#' @author Jae Hee Jung
#' @examples
...
#' regressionsObject <- evalRegressions(myX,myY)
#' show(regressionsObject)
#' @rdname showsubRegressions
#' @aliases show,subRegressions-method
#' @export
setMethod("show","subRegressions",function(object){
	for(i in 1:length(object@Rsquared)){
		...
		cat("For model",i,"pvalues are",paste(object@pvalues[i,],collapse=","),"\n\n")
	}
})